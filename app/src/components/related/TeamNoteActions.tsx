...
import { toggleTeamNoteDone, deleteTeamNote } from "../../store/notesApi";
import type { ObjectDetail } from "../../store/objectDetail";
import { zipHandles } from "./sections/shared";

/** Mark done/Reopen + delete for a Team Note, in RelatedPanel's
 * `view.key === "team_note"` branch -- same Button+Group+error-Alert shape
 ...
 * new state). */
export function TeamNoteActions({ detail, onToggled }: { detail: ObjectDetail; onToggled: () => void }) {
  ...
  async function handleToggle() {
    ...
    try {
      ...
      await toggleTeamNoteDone(token, detail.handle, !isDone);
  ...
  async function handleDelete() {
    ...
    try {
      const token = await getToken();
      await deleteTeamNote(token, detail.handle);
      setDeleted(true);
      getViewStore("team_note").requeryDebounced();
    } catch (err: any) {
      setError(err.message ?? String(err));
