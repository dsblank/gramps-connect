...
const HINTS: Record<string, Record<VisualKey, string>> = {
  person: {
...
const LABELS: Record<VisualKey, string> = { map: "Map", timeline: "Timeline" };

/** Map/Timeline buttons on their own row directly beneath a RelatedPanel's
 * title, for the four types a visual can be scoped to (hash.ts's
 * SUBJECT_KEYS).
 *
 * Below the title rather than in the header's action slot because these
 * aren't actions on the record -- they don't change it, they're two more
 * ways of looking at it, and they're the only two this panel offers. The
 * header slot is for Edit/Delete/Message, which do act on it.
 *
 ...
 * the URL and the visual re-derives the handles from the caches when it
 * gets there (store/visualScope.ts). Anchors rather than buttons so
 * middle-click, copy-link and Back all behave the way the rest of this
 * app's navigation does.
 *
 * Shown unconditionally on those four types, including where the record
 * turns out to have nothing to plot. Predicting that would mean resolving
