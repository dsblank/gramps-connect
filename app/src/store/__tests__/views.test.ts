...
import { BLOG_VIEW, GENERATED_VIEW, STORY_VIEW, TOPICS_VIEW, visibleColumns } from "../views";

// Output/Discussions/Stories/Blog are each identified by their own
// description or title, not a Gramps ID no one assigned them for any
// reason of their own -- hidden: true keeps the column selected/cached/
// searchable (each view's own simpleSearch still matches against it)
// without cluttering the table with an identifier nobody reads.
describe.each([
