...
echo "browser.ts's API_BASE/WS_URL already point here, but its MY_TREEID"
echo "constant is Postgres's own serial treeid for this fixture's tree (not"
echo "the UUID above) -- check it against:"
echo "  PGPASSWORD=gramps psql -h localhost -U gramps -d gramps -c \"SELECT treeid, uuid FROM trees;\" -- uuid == $TREE_UUID above"
echo "and update MY_TREEID in browser.ts if this is a from-scratch rebuild."
